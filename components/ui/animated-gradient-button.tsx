  ...
  const content = (
    <>
      ...
      <span className={cn("relative rounded-full transition-all duration-200 ease-out bg-gray-900 group-hover:bg-opacity-90 w-full h-full flex items-center justify-center", innerSizeClasses[size])}>
        {/* Enhanced mouse-tracking glass effect - disabled on mobile for performance */}
        {isHovered && (
          <span
            className="absolute w-20 h-20 bg-white/25 rounded-full blur-lg pointer-events-none hidden md:block"
            style={{
